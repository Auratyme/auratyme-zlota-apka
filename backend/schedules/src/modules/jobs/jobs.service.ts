...
import { TaskJobType } from '@app/common/constants';
      ...
      const cronJob = new CronJob(cronTime, () => {
        callback(...job.callbackParams)
          .then(() => {
            ...
            if (job.type === TaskJobType.SINGLE) {
              this.jobsRepository.removeById(job.id);
            }
          })
          .catch((err) => {
            this.logger.error(
    ...
    updateManyPromise.then((jobs) => {
      for (const job of jobs) {
        ...
        try {
          ...
          this.logger.error(
            new JobException('error while starting', err, true),
          );
        }
