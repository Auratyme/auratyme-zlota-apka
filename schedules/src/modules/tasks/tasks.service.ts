...
export class TasksService {
  constructor(
    ...
    const dueToJobCallback = async (id: string) => {
      this.eventsService.publish(TaskEvent.EXPIRED, { task: { id } });
    ...
    const repeatJobCallback = async (id: string) => {
      this.eventsService.publish(TaskEvent.REPEATED, { task: { id } });
    ...
    return this.jobsService
      ...
      .then((updatedJobs) => {
        if (updatedJobs.length <= 0) {
          return this.createAndScheduleDueToJob(taskId, taskDueTo);
        }
      });
  }
  ...
  private async rescheduleRepeatJob(
    ...
    return this.jobsService
      ...
      .then((updatedJobs) => {
        if (updatedJobs.length <= 0) {
          return this.createAndScheduleRepeatJob(taskId, taskRepeat);
        }
      });
  }
  ...
  async create(newTask: CreateTaskDto): Promise<Task> {
    ...
    createPromise.then((task) => {
      if (task.dueTo) {
        return this.createAndScheduleDueToJob(task.id, task.dueTo);
      }

      if (task.repeat) {
        return this.createAndScheduleRepeatJob(task.id, task.repeat);
      }
    });
  ...
  async updateOne(
    ...
    try {
      ...
      if (updatedTask.dueTo) {
        await this.rescheduleDueToJob(updatedTask.id, updatedTask.dueTo);
      }

      if (updatedTask.repeat) {
        await this.rescheduleRepeatJob(updatedTask.id, updatedTask.repeat);
      }
